...
def main():
    ...
    while a[instruction_code] != 99:
        ...
        if opcode == "4":
            ...
            if a[c_num] == 10 and last_output == 10:
                time.sleep(0.2)

            last_output = a[c_num]
            print(chr(a[c_num]), end="")
            instruction_code += 2
        elif opcode == "3":
            if not inputs:
                inputs = [ord(x) for x in input('--> ')] + [10]

            a[c_num] = inputs.pop(0)
            instruction_code += 2
        elif opcode == "1":
