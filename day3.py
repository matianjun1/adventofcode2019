...
first = first.split(",")
second = second.split(",")
grid = np.zeros((20000, 20000), dtype=int)
begin_point = [1000, 1000]
num = 1
for f in first:
    step = int(f[1:])
    if f[0] == "U":
        for i in range(begin_point[0] - 1, begin_point[0] - step - 1, -1):
            if grid[i, begin_point[1]] == 0:
                grid[i, begin_point[1]] = num
            num += 1
        begin_point[0] -= step
    if f[0] == "D":
        for i in range(begin_point[0] + 1, begin_point[0] + step + 1):
            if grid[i, begin_point[1]] == 0:
                grid[i, begin_point[1]] = num
            num += 1
        begin_point[0] += step
    if f[0] == "L":
        for i in range(begin_point[1] - 1, begin_point[1] - step - 1, -1):
            if grid[begin_point[0], i] == 0:
                grid[begin_point[0], i] = num
            num += 1
        begin_point[1] -= step
    if f[0] == "R":
        for i in range(begin_point[1] + 1, begin_point[1] + step + 1):
            if grid[begin_point[0], i] == 0:
                grid[begin_point[0], i] = num
            num += 1
        begin_point[1] += step
...
for s in second:
    ...
    if s[0] == "U":
        for i in range(begin_point[0] - 1, begin_point[0] - step - 1, -1):
            if grid[i, begin_point[1]] > 0:
                result.append(abs(i - 1000) + abs(begin_point[1] - 1000))
                result2.append(grid[i, begin_point[1]] + num)
            num += 1
        begin_point[0] -= step
    if s[0] == "D":
        for i in range(begin_point[0] + 1, begin_point[0] + step + 1):
    ...
    if s[0] == "L":
        for i in range(begin_point[1] - 1, begin_point[1] - step - 1, -1):
            if grid[begin_point[0], i] > 0:
                result.append(abs(begin_point[0] - 1000) + abs(i - 1000))
                result2.append(grid[begin_point[0], i] + num)
            num += 1
        begin_point[1] -= step
    if s[0] == "R":
        for i in range(begin_point[1] + 1, begin_point[1] + step + 1):
...
print("day3-1:", min(result))
print("day3-2:", min(result2))
